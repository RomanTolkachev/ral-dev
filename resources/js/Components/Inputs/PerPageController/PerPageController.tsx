                    ...
                    return (
                        <motion.div onAnimationComplete={handleAnimationComplete} ref={selectionsRef} onClick={() => setOpen(prev => !prev)}  animate={{y: open ? -20 : 0}}
                            className={`${open ? "!border-[rgb(var(--input-border-active))]" : ""}` + ` overflow-hidden min-h-8 h-fit left-[105%] absolute flex flex-col border-2 border-transparent items-center justify-center rounded-full py-0 font-semibold align-middle bg-input-primary text-sm w-12 text-input-text text-center shadow-input-page`}>
                                <motion.span className={"flex items-center select-none cursor-pointer"} initial={{height: active === "25" ?  32 : 0, opacity: active === "25" ?  1 : 0, }}  animate={{height: open || active === "25" ? "32px" : 0, opacity:  open || active === "25" ? 1 : 0}} onClick={(e:React.MouseEvent<HTMLSpanElement>) => {open && handleChange(e)}}>25</motion.span>
                                <motion.span className={"flex items-center select-none cursor-pointer"} initial={{height: active === "50" ?  32 : 0, opacity: active === "50" ?  1 : 0, }}  animate={{height: open || active === "50" ? "32px" : 0, opacity:  open || active === "50" ? 1 : 0}} onClick={(e:React.MouseEvent<HTMLSpanElement>) => {open && handleChange(e)}}>50</motion.span>
                                <motion.span className={"flex items-center select-none cursor-pointer"} initial={{height: active === "100" ?  32 : 0, opacity: active === "100" ?  1 : 0, }}  animate={{height: open || active === "100" ? "32px" : 0, opacity:  open || active === "100" ? 1 : 0}} onClick={(e:React.MouseEvent<HTMLSpanElement>) => {open && handleChange(e)}}>100</motion.span>
                        </motion.div>
                    )
                }}
            />
